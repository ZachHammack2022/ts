...
from data.mechanics import env_collision,player_collision
...
class BonkEnv(gym.Env):
    ...
    def __init__(self,render_mode = None):
        ...
        self.observation_space = spaces.Dict(
            {
                "agent": spaces.Dict(
                    {   
                        "x" : spaces.Box(low =100,high = 520, shape=(), dtype=np.float32),
                        "y" : spaces.Box(low =135,high = 420, shape=(), dtype=np.float32),
                        "x_v" : spaces.Box(dtype=np.float32),
                        "y_v" : spaces.Box(dtype=np.float32),
                        "alive" : spaces.Box(dtype=np.bool)
        ...
        # We have 4 actions, corresponding to "right", "up", "left", "down", right up right down, left up left down
        ...
        def reset(self, seed=None, options=None):
            ...
            self._agent_obs = dict(
                "x": np.random.random(121,499),
                "y": 200.0,
                "x_v" : 0,
                "y_v" : 0,
                "alive": True
            ...
            while abs(self._agent_obs["x"]-self._enemy_obs["x"]<25):
                self._enemy_obs["x"] = np.random.random(121,499)
        ...
        def step(self, action):
            ...
            if terminated:
                reward = 1 if  self.p1.alive # Binary sparse rewards
            reward = -0.01
            observation = self._get_obs()
            info = self._get_info()
